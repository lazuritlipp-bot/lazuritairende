...
def check_password():
    ...
    if not st.session_state.authenticated:
        ...
        st.markdown(
            f"""
            <style>
            [data-testid="stSidebar"], [data-testid="stSidebarCollapsedControl"], header, [data-testid="stHeader"], [data-testid="stToolbar"] {{
...
# --- РАБОЧАЯ ОБЛАСТЬ ---
# левую часть делаем уже, как вам нужно было ранее
col_left, col_main, col_hist = st.columns([2.0, 2.6, 0.6])
...
with col_left:
    ...
    CUSTOM_PATH = "svoi.png"

    # --- 4 иконки в ряд (функциональные, кликабельные) ---
    ...
    # --- Вытянутая кнопка "Свой промт" (функциональная) ---
    custom_cols = st.columns([1, 3, 1])
    with custom_cols[1]:
        selected_custom = image_select(
            ...
            return_value="index",
            key="custom_prompt_image_select",
        )
