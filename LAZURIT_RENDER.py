...
<style> .stApp { background-color: #E8E8E1; } .block-container { padding-top: 1rem !important; } .custom-header { background-color: white; padding: 10px 30px; border-radius: 12px; display:flex; justify-content: space-between; align-items:center; margin-bottom: 20px; border: 1px solid #D1D1D1; min-height: 90px; } .header-logo { height: 90px !important; width: auto !important; object-fit: contain; } .card { background-color: #F8F9FA; border-radius: 15px; padding: 20px; border: 1px solid #E0E0E0; margin-bottom: 15px; } .card > b { color:#000 !important; display:block; margin-bottom: 12px; } div.stButton > button:first-child[kind="primary"] { background: linear-gradient(90deg, #A78BFA 0%, #F87171 100%) !important; color: white !important; border: none !important; height: 55px !important; font-size: 18px !important; font-weight: bold !important; } .empty-result-card { height: 600px; display:flex; flex-direction:column; align-items:center; justify-content:center; color:#888; border:2px dashed #CCC; } /* Панель икон-кнопок */ .icon-grid { display: grid; grid-template-columns: repeat(3, 82px); gap: 14px; margin-top: 6px; margin-bottom: 10px; } .icon-btn { width: 82px; height: 82px; border-radius: 18px; background: #F2F2F2; border: 1px solid rgba(0,0,0,0.06); box-shadow: 6px 6px 14px rgba(0,0,0,0.18), -6px -6px 14px rgba(255,255,255,0.90); display: flex; align-items: center; justify-content: center; text-decoration: none; user-select: none; -webkit-tap-highlight-color: transparent; } .icon-btn img { width: 56px; height: 56px; object-fit: contain; display: block; } /* Не нужно сейчас (заглушено) .icon-btn:hover { transform: translateY(-1px); box-shadow: 8px 8px 18px rgba(0,0,0,0.20), -8px -8px 18px rgba(255,255,255,0.92); } .icon-btn:active { transform: scale(0.98); } .icon-btn.active { outline: 3px solid rgba(167,139,250,0.55); } */ </style>
""",
