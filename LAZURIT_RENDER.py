...
# Словарь: Название кнопки -> (Путь к иконке, Промпт)
# Это позволит нам легко управлять иконками и текстами
BUTTON_CONFIG = {
...
# Генерируем CSS для каждой кнопки с её иконкой
button_css = ""
for name, (img_path, _) in BUTTON_CONFIG.items():
    b64 = get_base64_image(img_path)
    if b64:
        # Ищем кнопку по тексту внутри неё и ставим фон
        button_css += f"""
        div.stButton button:has(span:contains("{name}")) {{
            background-image: url("data:image/png;base64,{b64}") !important;
            background-repeat: no-repeat !important;
            background-position: center !important;
            background-size: 60% !important; /* Размер иконки внутри кнопки */
            color: transparent !important; /* Скрываем текст, оставляем только иконку */
...
with col_left:
    ...
    p_cols = st.columns(3)
    for i, (name, (img, prompt_part)) in enumerate(BUTTON_CONFIG.items()):
        col = p_cols[i % 3]
        if col.button(name, key=f"btn_{i}"):
            # Формируем полный промпт: добавка + базовый
            full_prompt = f"{prompt_part} {BASE_PHOTO_PROMPT}" if prompt_part else ""
