Понял вас! Проблема с колонками в том, что они разбивают верстку Streamlit и ломают вашу красивую белую плашку.

Чтобы кнопка встала **именно туда, куда вы нарисовали (слева, прямо под текстом, внутри белой плашки)**, и при этом **вообще ничего не поплыло**, мы не будем трогать колонки. 

Мы сделаем хитрый и надежный трюк: оставим шапку единым целым блоком (как она и была изначально), а кнопку отрендерим сразу под ней и с помощью CSS «затянем» ее внутрь шапки на нужное место.

Ниже **полностью готовый код**. Скопируйте его целиком, он решает эту задачу идеально:
